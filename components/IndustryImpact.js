            ...
            <button
              ...
              className={`text-xs px-3 py-1 rounded transition-colors ${
                tab === t.key
                  ...
                  : 'text-gray-400 border border-[#1e3a5f] hover:text-gray-200'
              }`}
            >
              {t.label}
            </button>
...
function IndustryCard({ industry }) {
  ...
  return (
    <div
      className="rounded-lg bg-[#0d1b2a] border border-[#1e3a5f]/50 p-3 cursor-pointer hover:border-cyan-500/20 transition-colors"
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {expanded && (
            <div className="mt-3 pt-3 border-t border-[#1e3a5f]/50 space-y-2">
              ...
              {industry.companies?.length > 0 && (
                <div className="flex flex-wrap gap-1">
                  {industry.companies.map(c => (
                    <span key={c} className="text-[10px] px-1.5 py-0.5 rounded bg-[#111d32] text-gray-400 border border-[#1e3a5f]/30">
                      {c}
                    </span>
                  ))}
                </div>
              ...
              {industry.regions?.length > 0 && (
                <div className="flex flex-wrap gap-1">
                  {industry.regions.map(r => (
                    <span key={r} className="text-[10px] px-1.5 py-0.5 rounded bg-[#111d32] text-gray-400 border border-[#1e3a5f]/30">
                      {r}
                    </span>
                  ))}
                </div>
