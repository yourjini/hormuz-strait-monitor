...
function ShockGauge({ percent }) {
  ...
  return (
    <div>
      ...
      <div className="h-3 bg-[#1c1509] rounded-full overflow-hidden">
        <div
            ...
            <button
              ...
              className={`py-2 px-3 rounded-lg text-sm font-medium border transition-all ${
                inputs.fuelType === type
                  ...
                  : 'bg-[#1c1509] text-gray-400 border-[#6b5432] hover:text-gray-200'
              }`}
            >
              ...
              <button
                ...
                className={`flex items-center gap-2 p-2 rounded-lg text-left text-xs border transition-all ${
                  selected
                    ...
                    : 'bg-[#1c1509] border-[#6b5432] text-gray-400 hover:text-gray-300'
                }`}
              >
          ...
          {[1, 2, 3, 4, 5].map(n => (
            <button
              ...
              className={`flex-1 py-2 rounded-lg text-sm border transition-all ${
                inputs.household === n
                  ? 'bg-cyan-500/15 text-cyan-300 border-cyan-500/40'
                  : 'bg-[#1c1509] text-gray-400 border-[#6b5432]'
              }`}
            >
              {n}인
            </button>
...
function ResultPanel({ result, inputs, data, onBack }) {
  ...
  return (
    <div className="space-y-4 relative">
      ...
      {/* 총 추가 비용 (메인 결과) */}
      <div className="relative z-10 rounded-xl border-2 border-red-500/30 bg-gradient-to-br from-[#3a2d1c] to-[#1a1a2e] p-6 text-center overflow-hidden">
        {/* Background pulse */}
      ...
      {result.grocery.details.length > 0 && (
        <div className="relative z-10 rounded-lg border border-[#6b5432] bg-[#3a2d1c] p-4">
...
function ResultCard({ icon, title, diff, detail, delay }) {
  ...
  return (
    <div className={`rounded-lg border border-[#6b5432] bg-[#3a2d1c] p-4 transition-all duration-500 ${
  ...
  return (
    <div className={className}>
      ...
      <input
        ...
        className="w-full h-1.5 bg-[#6b5432] rounded-full appearance-none cursor-pointer accent-cyan-500
...
function ShareButtons({ result, onBack }) {
  ...
  const shareKakao = () => {
    // 모바일: Web Share API (카카오톡 앱 선택 가능)
    ...
    <div className="relative z-10 space-y-3">
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <button
          ...
          className="flex flex-col items-center gap-1.5 py-3 rounded-lg bg-[#6b5432] hover:bg-[#345580] transition-colors active:scale-[0.97]"
        >
      ...
      <button
        onClick={onBack}
        className="w-full py-3 rounded-lg border border-[#6b5432] text-gray-400 hover:text-gray-200 transition-colors text-sm"
      >
        ← 다시 입력하기
      </button>
    </div>
...
function Section({ title, desc, children }) {
  return (
    <div className="rounded-xl border border-[#6b5432] bg-[#3a2d1c] p-4">
      <div className="mb-3">
